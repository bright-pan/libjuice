#if !defined(JUICE_CONFIG_FILE)
#include "juice/juice_config.h"
#else
#include JUICE_CONFIG_FILE
#endif

#include <stdarg.h>
#include <stdio.h>

#include "rtp.h"
#include "sdp.h"

/*
    SDP语法可参考：
    https://webrtchacks.com/sdp-anatomy/
*/

int sdp_append(sdp_t *sdp, const char *format, ...) {

    va_list argptr;

    char attr[SDP_ATTR_LENGTH];

    memset(attr, 0, sizeof(attr));

    va_start(argptr, format);

    vsnprintf(attr, sizeof(attr), format, argptr);

    va_end(argptr);

    strcat(sdp->content, attr);
    strcat(sdp->content, "\r\n");
    return 0;
}

void sdp_reset(sdp_t *sdp) { memset(sdp->content, 0, sizeof(sdp->content)); }

void sdp_append_h264(sdp_t *sdp) {

    // sdp_append(sdp, "m=video 9 UDP/TLS/RTP/SAVPF 102 103");
    sdp_append(sdp, "m=video 9 UDP/TLS/RTP/SAVPF %d %d", RTP_PAYLOAD_TYPE_H264, RTP_PAYLOAD_TYPE_H264_RTX);
    sdp_append(sdp, "c=IN IP4 0.0.0.0");
    sdp_append(sdp, "a=mid:1");
    sdp_append(sdp, "a=sendonly");
    sdp_append(sdp, "a=rtcp-mux");
    sdp_append(sdp, "a=rtpmap:%d H264/90000", RTP_PAYLOAD_TYPE_H264);
    sdp_append(sdp, "a=rtcp-fb:%d goog-remb", RTP_PAYLOAD_TYPE_H264);
    sdp_append(sdp, "a=rtcp-fb:%d transport-cc", RTP_PAYLOAD_TYPE_H264);
    sdp_append(sdp, "a=rtcp-fb:%d ccm fir", RTP_PAYLOAD_TYPE_H264);
    sdp_append(sdp, "a=rtcp-fb:%d nack", RTP_PAYLOAD_TYPE_H264);
    sdp_append(sdp, "a=rtcp-fb:%d nack pli", RTP_PAYLOAD_TYPE_H264);
    sdp_append(sdp, "a=fmtp:%d level-asymmetry-allowed=1;packetization-mode=1;profile-level-id=42001f", RTP_PAYLOAD_TYPE_H264);
    // sdp_append(sdp, "a=ssrc:9527 cname:webrtc-h264");
    sdp_append(sdp, "a=rtpmap:%d rtx/90000", RTP_PAYLOAD_TYPE_H264_RTX);
    sdp_append(sdp, "a=fmtp:%d apt=%d", RTP_PAYLOAD_TYPE_H264_RTX, RTP_PAYLOAD_TYPE_H264);
    // sdp_append(sdp, "a=ssrc:9528 cname:webrtc-h264-rtx");
    sdp_append(sdp, "a=ssrc-group:FID %d %d", RTP_SSRC_TYPE_H264, RTP_SSRC_TYPE_H264_RTX);
}

void sdp_append_pcma(sdp_t *sdp) {

    /*
        https://www.ietf.org/archive/id/draft-garcia-mmusic-multiple-ptimes-problem-02.html

        maxptime specify the maximum amount of media that can be encapsulated in each packet,
        expressed as time in milliseconds. The size of the packet can have side effects in the
        quality of the audio and the BW. It is possible to modify this values in the SDP.

        PCMA default：160samples/20ms, 160bytes/packet
        800samples/100ms, 800bytes/packet
    */

    sdp_append(sdp, "m=audio 9 UDP/TLS/RTP/SAVPF 8");
    sdp_append(sdp, "c=IN IP4 0.0.0.0");
    sdp_append(sdp, "a=mid:0");
    sdp_append(sdp, "a=sendrecv");
    sdp_append(sdp, "a=rtcp-mux");
    sdp_append(sdp, "a=rtpmap:8 PCMA/8000");
    sdp_append(sdp, "a=ptime:60");
    sdp_append(sdp, "a=ssrc:4 cname:webrtc-pcma");
}

void sdp_append_pcmu(sdp_t *sdp) {

    sdp_append(sdp, "m=audio 9 UDP/TLS/RTP/SAVPF 0");
    sdp_append(sdp, "a=rtpmap:0 PCMU/8000");
    sdp_append(sdp, "a=ssrc:5 cname:webrtc-pcmu");
    sdp_append(sdp, "a=sendrecv");
    sdp_append(sdp, "a=mid:1");
    sdp_append(sdp, "c=IN IP4 0.0.0.0");
    sdp_append(sdp, "a=rtcp-mux");
}

void sdp_append_datachannel(sdp_t *sdp) {

    sdp_append(sdp, "m=application 9 UDP/DTLS/SCTP webrtc-datachannel");
    sdp_append(sdp, "a=mid:2");
    sdp_append(sdp, "a=sctp-port:5000");
    sdp_append(sdp, "c=IN IP4 0.0.0.0");
    sdp_append(sdp, "a=max-message-size:262144");
}

void sdp_create(sdp_t *sdp, int b_video, int b_audio, int b_datachannel) {

    char bundle[64];
    sdp_append(sdp, "v=0");
    sdp_append(sdp, "o=- 1495799811084970 1495799811084970 IN IP4 0.0.0.0");
    sdp_append(sdp, "s=-");
    sdp_append(sdp, "t=0 0");
    sdp_append(sdp, "a=msid-semantic: iot");

    memset(bundle, 0, sizeof(bundle));

    strcat(bundle, "a=group:BUNDLE");

    if (b_audio) {
        strcat(bundle, " 0");
    }
    if (b_video) {
        strcat(bundle, " 1");
    }
    if (b_datachannel) {
        strcat(bundle, " 2");
    }




    sdp_append(sdp, bundle);
}
